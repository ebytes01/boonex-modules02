...
class EmmetBytesClubCoverModule extends BxDolTwigModule {
        ...
        if(isset($_POST) && sizeof($_POST) > 0){
            ...
            if($authorId == $loggedId){
                if(isset($_POST['stream_url']) && !empty($_POST['stream_url'])){
                    ...
                    if($streamUrl != _t('_emmetbytes_club_cover_stream_player_url_input_caption')){
    ...
    private function getClubLogoFullInfo(){
        ...
        if(!file_exists($clubLogoDir)){
            $clubModule = BxDolModule::getInstance('BxClubModule');
            $clubLogo = $clubModule->_oTemplate->getImageUrl('no-image-thumb.png');
            $clubLogoDir = $clubModule->_oTemplate->getImagePath('no-image-thumb.png');
        }
    ...
    function getClubRate(){
        ...
        return array($o->getBigVoting($this->isAllowedRate($this->clubDatas)));
    }
    ...
    function isAllowedRate() {        
        if ($this->isAdmin())
            return true;
        $clubModule = BxDolModule::getInstance('BxClubModule');
        return $clubModule->isAllowedRate($this->clubDatas);
    }
    ...
    protected function getAlertLanguages($type){
        return array(
            'add_background' => '_eb_club_cover_add_' . $type . '_post_background',
            'change_background' => '_eb_club_cover_change_' . $type . '_post_background',
            'repositioned_background' => '_eb_club_cover_repositioned_' . $type . '_post_background',
            'remove_background' => '_eb_club_cover_remove_' . $type . '_post_background',
            'add_thumbnail' => '_eb_club_cover_add_' . $type . '_post_thumbnail',
            'change_thumbnail' => '_eb_club_cover_change_' . $type . '_post_thumbnail',
        );
    }
    ...
    function getSpyPost($sAction, $iObjectId, $iSenderId, $aExtraParams){
        ...
        return $this->_serviceGetSpyPost($sAction, $iObjectId, $iSenderId, $aExtraParams, $lang);
    }
    ...
    private function alert($alertParams){
        ...
        $oAlert = new BxDolAlerts($this->_sPrefix, $action, $entryId, $profileId, array('status' => $status)); 
        $oAlert->alert();
    }
}
