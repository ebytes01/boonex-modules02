...
/*
 * ProfileCover module Data
 */
class EmmetBytesProfileCoverDb extends BxDolTwigModuleDb {	

	function EmmetBytesProfileCoverDb(&$oConfig) {
		parent::BxDolTwigModuleDb($oConfig);
        $this->_sTableMain = '_main';
        $this->_sPrefix = 'eb_profile_cover';
        $this->_sTableId = 'id';
    ...
    function removeBackgroundProfileCover($id){
        return $this->query("
            DELETE FROM
                `{$this->_sPrefix}{$this->_sTableMain}`
            WHERE
